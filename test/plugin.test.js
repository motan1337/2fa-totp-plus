'use strict'
// emulates the parts of hfs 3.3.2 the plugin interacts with (src/auth.ts setLoggedIn, src/plugins.ts listener trap and
// middleware, src/api.auth.ts login/loginSrp2, src/middlewares.ts prepareState), and checks that 2FA is really enforced.
// older hfs are covered by 2fa-totp-legacy-plus
...
async function setup() {
    ...
    const api = {
        Const: { API_URI: '/~/api/', API_VERSION: 13.5 },
    ...
    // auth.ts
    async function setLoggedIn(ctx, username, via) {
        const s = ctx.session
        ...
        const result = await emitAsync('finalizingLogin', { ctx, username: a.username, via, inputs })
        const error = result.find(x => x && typeof x === 'string')
        if (error) {
            ctx.state.account = getAccount(s.username)
            delete ctx.state.usernames
            throw Object.assign(new Error(error), { status: 401, expose: true }) // like koa ctx.throw
        }
    ...
    function ctxFor({ query = {}, params, authorization = '', autoLogin, session = {} } = {}) {
        return {
            query, session, autoLogin, state: { params }, ip: '1.2.3.4', status: 404, body: undefined,
    ...
    async function request(ctx, apiHandler) {
        ...
        const byHeader = u && u !== s.username && getAccount(u)
        let a = byHeader || !s.username && getAccount(ctx.autoLogin)
        const loggedInNotBySession = a
        ctx.state.account = a ||= getAccount(s.username)
        if (a && loggedInNotBySession)
            try {
                await setLoggedIn(ctx, a.username, byHeader ? 'header' : 'net')
            }
            catch (e) {
                emit('failedLogin')
                return Object.assign(ctx, { status: 401, body: e.message })
            }
    ...
    const loginApi = username => async ctx => {
        try {
            await setLoggedIn(ctx, getAccount(username).username, 'body')
        }
        catch (e) {
            emit('failedLogin')
    ...
    // another plugin logging the user in, hfs passes no via
    const pluginLogin = username => async ctx => {
        await setLoggedIn(ctx, username)
        return { status: 200, body: 'ok' }
    }
    const loginCtx = (otp, session = {}) => ctxFor({ params: otp === undefined ? {} : { otp }, session })

    return { api, pl, store, config, logs, fired, emit, request, loginApi, srpApi, pluginLogin, ctxFor, loginCtx }
}
...
test('SRP login with the username in a different case still requires the OTP', async () => {
    const t = await setup()
    t.store.set('alice', { base32: SECRET })
    const srpCtx = otp => t.ctxFor({ params: { otp }, session: { loggingIn: { username: 'ALICE' } } })
    const bad = await t.request(srpCtx(''), t.srpApi)
    assert.equal(bad.status, 401)
    assert.equal(bad.body, 'OTP required')
...
test('basic auth without ?otp= is refused with a 401 error', async () => {
    ...
    assert.equal(refused.body, 'OTP required')
...
test('auto_login_net logins are not refused, but undone', async () => {
    const t = await setup()
    t.store.set('alice', { base32: SECRET })
    const auto = await t.request(t.ctxFor({ autoLogin: 'alice' }))
...
test('a login made by another plugin is undone, and answered with 401', async () => {
    const t = await setup()
    t.store.set('alice', { base32: SECRET })
    const ctx = await t.request(t.ctxFor(), t.pluginLogin('alice'))
    assert.equal(ctx.status, 401)
    assert.equal(ctx.body, 'OTP required')
    assert.equal(ctx.state.account, undefined)
    assert.equal(ctx.session.username, undefined)
})
