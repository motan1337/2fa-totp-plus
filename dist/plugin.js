...
exports.init = async api => {
    ...
    api.events.on('finalizingLogin', async ({ ctx, username: user, inputs, via }) => {
        // via is net for auto_login_net, and missing when the login is made by another plugin
        const hard = Boolean(via) && via !== 'net'
    ...
    function denyLogin(ctx, message, hard, mark) {
        ...
        const s = ctx.session
        // auto_login_net and logins made by other plugins are let through, and our middleware undoes them
        // refusing an auto_login_net login would answer 401 to the whole network, and antibrute would block it
        if (!hard || !s) return
        if (mark !== undefined)
